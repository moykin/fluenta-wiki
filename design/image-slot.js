...
(() => {
  ...
  // Keep the utm_source value identical everywhere it is emitted below.
  const UNSPLASH_HOMEPAGE_HREF =
    'https://unsplash.com/?utm_source=fluenta&utm_medium=referral';
  ...
  const withReferral = (href) => {
    try {
      ...
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'fluenta');
      }
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // (written into the HTML by the mockup itself) so it passes through
      // unchanged.
